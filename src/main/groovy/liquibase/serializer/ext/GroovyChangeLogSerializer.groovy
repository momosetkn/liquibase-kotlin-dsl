 ...
 * must be in the liquibase.serializer.ext package to be found by Liquibase at runtime.
 *
 * @author Tim Berglund
 ...
 * @param pretty whether or not to make it pretty.  It doesn't matter what you pass here because
 *        this DSL refuses to make a serialization that isn't pretty.
 ...
 */
  ...
  for the new changelog, then copy and paste that content into the existing file.""")
    ...
    //--------------------------------------------------------------------------------------------
    // In Liquibase 2.x, there were many different serialize methods for different types of
    // liquibase objects. In version 3.0, the different serializable objects all implement a common
    // interface, and the serialize methods were replaced by a single method.  However, we want to
    // do different things with different objects with respect to ordering of attributes, etc.
    // Therefore, certain objects have their own serialize methods which will be called by the
    // master method.  These methods are private to force outside classes to use serialization
...
${renderedChildren}
    ...
    private String serializeObject(Change change) {
        ...
        fields.each { field ->
            ...
            // TODO: The TextNode annotation used to mark a special case where we set the text body
            // to the field value.  This only applied to UpdateDataChange and DeleteDataChange.  we
            // need to figure out what to do with these classes in 3.0
...
${serializedChange} {
    ...
    /**
     * Builds the correct string representation of an object's properties based on the property's
     * type.
