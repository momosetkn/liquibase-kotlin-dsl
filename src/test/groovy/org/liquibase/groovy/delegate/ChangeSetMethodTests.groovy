...
/**
 * This class tests the methods of a change set that are neither preconditions, nor refactorings.
 * This includes things like {@code rollback}
 *
...
class ChangeSetMethodTests extends ChangeSetTests {
    ...
    /**
     * Test the validChecksum functionality.  This test needs some explanation. Liquibase's
     * {@code isChecksumValid( )} method compares the change set's current checksum to the hash
     * given to the method. If they don't match, it will check the current checksum against
     * checksums that are stored with the validChecksum element.  So to test this, we do the
     * following:<br>
     * <ol>
     * <li>Call isChecksumValid with a bogus checksum.  Because the bogus checksum does not match
     * the current checksum, we expect it to return false.</li>
     * <li>apply the validCheckSum method with the current, valid, checksum for the changeSet.</li>
     * <li>Call isChecksumValid with the same bogus checksum as before. It still won't match the
     * current calculated checksum, but since that checksum has been marked as valid, we should now
     * get a true result.</li>
     * </ol>
     */
    @Test
    ...
    /**
     * Test a rollback with a single statement passed as a string.  In this test, we expect a null
     * resource accessor because they are not needed for rollbacks that are just SQL, and Liquibase
     * doesn't give us an easy way to pass in a resource accessor.
     */
    @Test
    void rollbackString() {
    ...
    /**
     * Test rollback with two statements passed as strings. In this test, we expect a null resource
     * accessor because they are not needed for rollbacks that are just SQL, and Liquibase doesn't
     * give us an easy way to pass in a resource accessor.
     */
    @Test
...
ALTER TABLE monkey_table DROP COLUMN angry;"""
    ...
    /**
     * Rollback one statement given in a closure.  In this test, we expect a null resource accessor
     * because they are not needed for rollbacks that are just SQL, and Liquibase doesn't give us an
     * easy way to pass in a resource accessor.
     */
    @Test
    ...
    /**
     * Rollback two statements given in a closure. In this test, we expect a null resource accessor
     * because they are not needed for rollbacks that are just SQL, and Liquibase doesn't give us an
     * easy way to pass in a resource accessor.
     */
    @Test
...
ALTER TABLE monkey_table DROP COLUMN angry;"""
    ...
    /**
     * Rollback with two changes when they are both given as nested refactorings.  We don't care too
     * much about the content of the resultant changes, just that the right changes of the right
     * type wind up in the right place.
     */
    @Test
    ...
    /**
     * Process a map based rollback that is missing the changeSetId.  Expect an error.
     */
    ...
    /**
     * Process a map based rollback when we don't supply a file path.  In that case, we should use
     * the one in the databaseChangeLog.  This test needs to use the same constants in the rollback
     * as was used to create the changeSet in the set up.
     */
    @Test
    ...
    /**
     * Test a change with an invalid attribute.  Since all changes funnel through the same method to
     * process the attributes, we only need to test this once.  For this test, we've chosen the
