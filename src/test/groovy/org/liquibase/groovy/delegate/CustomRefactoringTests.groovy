 ...
 * particular class tests custom changes such as{@code sql} and {@code executeCommand}
 * <p>
 * Since the Kotlin DSL parser is meant to act as a pass-through for Liquibase itself, it doesn't do
...
class CustomRefactoringTests extends ChangeSetTests {
    ...
    /**
     * Test parsing an executeCommand with no args and an empty closure to make sure the DSL doesn't
    ...
    /**
     * Test parsing an executeCommand change when we have no attributes and there is no closure.
     */
    @Test
    ...
    /**
     * Test parsing executeCommand when we have all supported attributes,but no argument closure.
     */
    @Test
    ...
    /**
     * Test parsing an executeCommand change where the arguments are maps, like the XML would do it.
     */
    @Test
    ...
    /**
     * Test parsing an executeCommand change where the arguments are just Strings. This is not the
     * way the XML does it, but it is the way the Kotlin DSL has always done it, and it is nice
     * shorthand.
     */
    @Test
    ...
    /**
     * Make sure modifySql works.  Most of the tests for this are in {@link ModifySqlDelegateTests},
     * this just needs to make sure that the SqlVisitors that the delegate returns are added to the
     * changeSet.  This one also tests that we can have a modifySql with no attributes of its own.
     */
    @Test
    void modifySqlValid() {
        buildChangeSet {
     ...
     * sure we don't get any unintended defaults. Also test our assumption that Liquibase will
     * default splitStatements to true and stripComments to false.
     */
    @Test
    ...
    /**
     * Test parsing a sql change when we have no attributes or a closure, just a string.
     */
    @Test
    void sqlIsString() {
        buildChangeSet {
    ...
    /**
     * Test parsing a sql change when we have no attributes, but we do have a comment in the
     * closure.
     */
    @Test
    ...
    /**
     * Test parsing a sql change when we have all attributes and we have a comment in the closure.
     * For this test we only set splitStatements to true.
     */
    @Test
    ...
    /**
     * Test parsing a sqlFile change with minimal attributes to confirm our assumptions about
     * Liquibase defaults, which we assume to be true for splitStatements and false for
     * stripComments.  We can't test this one with totally empty attributes because a sqlFile change
     * will attempt to open the file immediately to work around a Liquibase bug.  This also means
     * the file in question must exist.
     */
    @Test
    void sqlFileEmpty() {
        buildChangeSet {
